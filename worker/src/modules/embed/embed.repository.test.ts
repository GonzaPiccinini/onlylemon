 ...
 * - EMBED_SELECT projects only public fields: id, status, whatsappMessages, metaPixelRelation.pixelId
...
test('EMBED_SELECT does not include accessToken key in the metaPixelRelation select', async () => {
  const { EMBED_SELECT } = await import('./embed.repository.js');

  assert.ok(EMBED_SELECT.metaPixelRelation !== undefined, 'EMBED_SELECT must have metaPixelRelation');
  const nestedSelect = (
    EMBED_SELECT.metaPixelRelation as { select: Record<string, unknown> }
  ).select;
  assert.ok(
    !('accessToken' in nestedSelect),
    'EMBED_SELECT.metaPixelRelation.select must NOT have accessToken',
  );
});

test('EMBED_SELECT includes exactly the public fields: id, status, whatsappMessages, metaPixelRelation with pixelId', async () => {
  const { EMBED_SELECT } = await import('./embed.repository.js');
  ...
  assert.equal(nestedSelect.pixelId, true, 'metaPixelRelation.select must have pixelId: true');
  assert.ok(!('id' in nestedSelect), 'metaPixelRelation.select must NOT expose MetaPixel.id (unnecessary)');
});

test('EMBED_SELECT metaPixelRelation nested select only exposes pixelId (no label, no createdAt)', async () => {
  const { EMBED_SELECT } = await import('./embed.repository.js');
  ...
  assert.deepEqual(nestedKeys, ['pixelId'], `metaPixelRelation.select should only have ["pixelId"], got: ${JSON.stringify(nestedKeys)}`);
});
